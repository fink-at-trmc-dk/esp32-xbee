...
#define STATUS_LED_GREEN_GPIO GPIO_NUM_21
#define STATUS_LED_BLUE_GPIO GPIO_NUM_21
...
static SLIST_HEAD(status_led_color_list_t,
                  status_led_color_t) status_led_colors_list;

static TaskHandle_t led_task;

void status_led_clear() {}

status_led_handle_t status_led_add(uint32_t rgba,
...
static void status_led_channel_fade(ledc_channel_t channel, uint8_t value,
...
static void status_led_fade(uint8_t red, uint8_t green, uint8_t blue,
                            int max_fade_time_ms) {
  status_led_channel_fade(STATUS_LED_RED_CHANNEL, 0xFF - red, max_fade_time_ms);
  status_led_channel_fade(STATUS_LED_GREEN_CHANNEL, 0xFF - green,
                          max_fade_time_ms);
  status_led_channel_fade(STATUS_LED_BLUE_CHANNEL, 0xFF - blue,
                          max_fade_time_ms);
}
...
static void status_led_show(status_led_handle_t color) {
  if (color->flashing_mode == STATUS_LED_STATIC) {
    ...
    bool active = true;
    for (unsigned int i = 0; i < color->duration / color->interval;
         i++, active = !active) {
...
void status_led_init() {
  ...
  ledc_channel_config_t ledc_config = {.duty = 255,
                                       .speed_mode = LEDC_SPEED_MODE,
                                       .hpoint = 0,
  ...
  xTaskCreate(status_led_task, "status_led", 2048, NULL,
              TASK_PRIORITY_STATUS_LED, &led_task);
}
...
void sleep_led_fade(uint8_t value, int max_fade_time_ms) {
  status_led_channel_fade(STATUS_LED_SLEEP_CHANNEL, 0xFF - value,
                          max_fade_time_ms);
}