...
//         if (xQueueReceive(button_queue, &button_ev, 1000 /
//         portTICK_PERIOD_MS)) {
...
void app_main() {
  status_led_init();
  status_led_handle_t status_led =
      status_led_add(0xFFFFFF33, STATUS_LED_FADE, 250, 2500, 0);
  ...
  //   xTaskCreate(reset_button_task, "reset_button", 4096, NULL,
  //               TASK_PRIORITY_RESET_BUTTON, NULL);
  ...
  uart_nmea("$PESP,INIT,START,%s,%s", app_desc->version,
  ...
  if (reset_reason != ESP_RST_POWERON && reset_reason != ESP_RST_SW &&
    ...
    status_led_handle_t error_led =
        status_led_add(0xFF000033, STATUS_LED_BLINK, 50, 10000, 0);
