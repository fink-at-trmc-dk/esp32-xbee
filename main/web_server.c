/*
 * This file is part of the ESP32-XBee distribution
 * (https://github.com/nebkat/esp32-xbee). Copyright (c) 2019 Nebojsa Cvetkovic.
 *
...
static esp_err_t www_spiffs_init() {
  ...
  esp_vfs_spiffs_conf_t conf = {.base_path = WWW_PARTITION_PATH,
...
static char *get_path_from_uri(char *dest, const char *base_path,
...
static esp_err_t json_response(httpd_req_t *req, cJSON *root) {
  ...
  if (!success) {
    ...
    httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR,
                        "Not enough space in buffer to output JSON");
    return ESP_FAIL;
  }
...
static esp_err_t basic_auth(httpd_req_t *req) {
  int authorization_length =
      httpd_req_get_hdr_value_len(req, "Authorization") + 1;
  if (authorization_length == 0)
    goto _auth_required;

  char *authorization_header = malloc(authorization_length);
  httpd_req_get_hdr_value_str(req, "Authorization", authorization_header,
                              authorization_length);

  bool authenticated =
      strcasecmp(basic_authentication, authorization_header) == 0;
...
static esp_err_t hotspot_auth(httpd_req_t *req) {
  ...
  // ERROR_ACTION(TAG, client_addr.sin6_family != AF_INET, goto _auth_error,
  // "IPv6 connections not supported, IP family %d", client_addr.sin6_family);
  ...
  for (int i = 0; i < ap_sta_list_mac_ip.num; i++) {
    if (ap_sta_list_mac_ip.sta[i].ip.addr ==
        client_addr.sin6_addr.un.u32_addr[3])
      return ESP_OK;
  }

  //_auth_error:
  httpd_resp_set_status(req, "401"); // Unauthorized
  char *unauthorized = "401 Unauthorized - Configured to only accept "
                       "connections from hotspot devices";
  httpd_resp_send(req, unauthorized, strlen(unauthorized));
  return ESP_FAIL;
}
...
static esp_err_t core_dump_get_handler(httpd_req_t *req) {
  ...
  char date[20] = "\0";
  if (t > 315360000l)
    strftime(date, sizeof(date), "_%F_%T", localtime(&t));

  char content_disposition[128];
  snprintf(content_disposition, sizeof(content_disposition),
           "attachment; filename=\"esp32_xbee_%s_core_dump_%s%s.bin\"",
...
static esp_err_t heap_info_get_handler(httpd_req_t *req) {
  ...
  cJSON_AddNumberToObject(root, "total_allocated_bytes",
...
static esp_err_t file_check_etag_hash(httpd_req_t *req, char *file_hash_path,
                                      char *etag, size_t etag_size) {
  ...
  ERROR_ACTION(
      TAG, fd_hash == NULL, return ESP_FAIL,
      "Could not open hash file %s (%lu bytes) for reading/updating: %d %s",
      file_hash_path, file_hash_stat.st_size, errno, strerror(errno));
  ...
  ERROR_ACTION(TAG, read != 1, return ESP_FAIL,
               "Could not read hash file %s: %d %s", file_hash_path, errno,
               strerror(errno));

  snprintf(etag, etag_size, "\"%08X\"", (unsigned int)crc);

  // Compare to header sent by client
  size_t if_none_match_length =
      httpd_req_get_hdr_value_len(req, "If-None-Match") + 1;
  if (if_none_match_length > 1) {
    char *if_none_match = malloc(if_none_match_length);
    httpd_req_get_hdr_value_str(req, "If-None-Match", if_none_match,
                                if_none_match_length);
    ...
    if (header_match) {
      ...
      ESP_LOGW(TAG, "ETag for file %s sent by client does not match (%s != %s)",
               file_hash_path, etag, if_none_match);
      free(if_none_match);
...
static esp_err_t file_get_handler(httpd_req_t *req) {
  ...
  char *file_name = get_path_from_uri(file_path, WWW_PARTITION_PATH, req->uri,
                                      sizeof(file_path));
  ERROR_ACTION(
      TAG, file_name == NULL,
      {
  ...
  if (file_name[strlen(file_name) - 1] == '/' &&
      strlen(file_name) + strlen("index.html") < FILE_PATH_MAX) {
  ...
  ERROR_ACTION(
      TAG, stat(file_path, &file_stat) == -1,
      {
  ...
  if (fd_hash != NULL) {
    ...
    ESP_LOGW(TAG, "Could not open hash file %s for writing: %d %s",
             file_hash_path, errno, strerror(errno));
  }
...
static esp_err_t config_get_handler(httpd_req_t *req) {
  ...
  for (int i = 0; i < config_item_count; i++) {
    ...
    case CONFIG_ITEM_TYPE_COLOR:
      ...
      asprintf(&string, "#%02x%02x%02x", color.values.red, color.values.green,
               color.values.blue);
      break;
    case CONFIG_ITEM_TYPE_IP:
      ...
      for (int b = 0; b < 4; b++) {
        cJSON_AddItemToArray(ip_parts,
                             cJSON_CreateNumber(esp_ip4_addr_get_byte(&ip, b)));
      }
    ...
    if (string != NULL) {
      ...
      char *value =
          item->secret && strlen(string) > 0 ? CONFIG_VALUE_UNCHANGED : string;
  ...
  for (int i = 0; i < config_item_count; i++) {
    ...
    if (cJSON_HasObjectItem(root, item.key)) {
      ...
      if (cJSON_IsString(entry)) {
        ...
        if (strcmp(entry->valuestring, CONFIG_VALUE_UNCHANGED) == 0)
          continue;
      }
      ...
      if (item.type > CONFIG_ITEM_TYPE_MAX) {
        ...
        if (!is_black && color.rgba == 0) {
          ...
          if (!is_black)
            color.values.alpha = item.def.color.values.alpha;
        ...
        if (!cJSON_IsArray(entry) || cJSON_GetArraySize(entry) != 4) {
          ...
          for (int b = 0; b < 4; b++) {
            a[b] = (uint8_t)strtoul(cJSON_GetArrayItem(entry, b)->valuestring,
                                    NULL, 10);
          };
          uint32_t ip =
              esp_netif_htonl(esp_netif_ip4_makeu32(a[0], a[1], a[2], a[3]));
          err = config_set_u32(item.key, ip);
        }
      } else {
        bool is_zero = strcmp(entry->valuestring, "0") == 0 ||
                       strcmp(entry->valuestring, "0.0") == 0;
      ...
      if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error setting %s = %s: %d - %s", item.key,
                 entry->valuestring, err, esp_err_to_name(err));
      }
    }
  }
...
static esp_err_t status_get_handler(httpd_req_t *req) {
  ...
  cJSON_AddNumberToObject(root, "uptime",
                          (int)((double)esp_timer_get_time() / 1000000));

  // Heap
  cJSON *heap = cJSON_AddObjectToObject(root, "heap");
  cJSON_AddNumberToObject(heap, "total",
                          heap_caps_get_total_size(MALLOC_CAP_8BIT));
  cJSON_AddNumberToObject(heap, "free",
                          heap_caps_get_free_size(MALLOC_CAP_8BIT));

  // Streams
  cJSON *streams = cJSON_AddObjectToObject(root, "streams");
  stream_stats_values_t values;
  for (stream_stats_handle_t stats = stream_stats_first(); stats != NULL;
       stats = stream_stats_next(stats)) {
  ...
  for (int s = LWIP_SOCKET_OFFSET;
       s < LWIP_SOCKET_OFFSET + CONFIG_LWIP_MAX_SOCKETS; s++) {
    int err;
    ...
    if (err == 0)
      cJSON_AddStringToObject(socket, "local",
                              sockaddrtostr((struct sockaddr *)&addr));

    err = getpeername(s, (struct sockaddr *)&addr, &socklen);
    if (err == 0)
      cJSON_AddStringToObject(socket, "peer",
                              sockaddrtostr((struct sockaddr *)&addr));
  ...
  if (ap_status.active) {
    ...
    cJSON_AddStringToObject(ap, "authmode",
                            wifi_auth_mode_name(ap_status.authmode));
  ...
  if (sta_status.active) {
    ...
    if (sta_status.connected) {
      ...
      cJSON_AddStringToObject(sta, "authmode",
                              wifi_auth_mode_name(sta_status.authmode));
...
static esp_err_t wifi_scan_get_handler(httpd_req_t *req) {
  ...
  for (int i = 0; i < ap_count; i++) {
    ...
    cJSON_AddStringToObject(ap, "authmode",
                            wifi_auth_mode_name(ap_record->authmode));
  }
...
static esp_err_t register_uri_handler(httpd_handle_t server, const char *path,
                                      httpd_method_t method,
                                      esp_err_t (*handler)(httpd_req_t *r)) {
  httpd_uri_t uri_config_get = {
      .uri = path, .method = method, .handler = handler};
...
static httpd_handle_t web_server_start(void) {
  ...
  if (auth_method == AUTH_METHOD_BASIC) {
    ...
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_ADMIN_USERNAME),
                              (void **)&username);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_ADMIN_PASSWORD),
                              (void **)&password);
