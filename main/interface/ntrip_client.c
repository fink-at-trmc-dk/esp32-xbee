...
static void nmea_gga_extract(int32_t length, void *buffer) {
  ...
  void *end =
      memmem(start, length - (start - buffer), GGA_END, strlen(GGA_END));
...
static void ntrip_client_task(void *ctx) {
  ...
  config_color_t status_led_color =
      config_get_color(CONF_ITEM(KEY_CONFIG_NTRIP_CLIENT_COLOR));
  ...
  while (true) {
    ...
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_CLIENT_HOST),
                              (void **)&host);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_CLIENT_USERNAME),
                              (void **)&username);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_CLIENT_PASSWORD),
                              (void **)&password);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_CLIENT_MOUNTPOINT),
                              (void **)&mountpoint);
    ...
    snprintf(buffer, BUFFER_SIZE,
             "GET /%s HTTP/1.1" NEWLINE "User-Agent: NTRIP %s/%s" NEWLINE
             "Authorization: %s" NEWLINE NEWLINE,
             mountpoint, NTRIP_CLIENT_NAME,
             &esp_app_get_description()->version[1], authorization);
    free(authorization);
    ...
    ERROR_ACTION(
        TAG, status == NULL || !ntrip_response_ok(status), free(status);
        goto _error, "Could not connect to mountpoint: %s",
        status == NULL
            ? "HTTP response malformed"
            : (ntrip_response_sourcetable_ok(status) ? "Mountpoint not found"
                                                     : status))
    free(status);
    ...
    xTaskCreate(ntrip_client_nmea_gga_send_task, "ntrip_client_gga", 2048, NULL,
                TASK_PRIORITY_INTERFACE, &nmea_gga_send_task);
...
void ntrip_client_init() {
  ...
  xTaskCreate(ntrip_client_task, "ntrip_client_task", 4096, NULL,
