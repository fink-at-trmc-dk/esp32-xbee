...
static void ntrip_server_uart_handler(void *handler_args, esp_event_base_t base,
  ...
  if ((event_bits & DATA_READY_BIT) == 0) {
    ...
    if (event_bits & DATA_SENT_BIT)
      ESP_LOGI(TAG, "Data received by UART, will now reconnect to caster if "
                    "disconnected");
  }
  data_keep_alive = 0;
...
static void ntrip_server_task(void *ctx) {
  ...
  xTaskCreate(ntrip_server_sleep_task, "ntrip_server_sleep_task", 2048, NULL,
              TASK_PRIORITY_INTERFACE, &sleep_task);

  config_color_t status_led_color =
      config_get_color(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER_COLOR));
  if (status_led_color.rgba != 0)
    status_led =
        status_led_add(status_led_color.rgba, STATUS_LED_FADE, 500, 2000, 0);
  if (status_led != NULL)
  ...
  while (true) {
    ...
    if ((xEventGroupGetBits(server_event_group) & DATA_READY_BIT) == 0) {
      ...
      xEventGroupWaitBits(server_event_group, DATA_READY_BIT, true, false,
                          portMAX_DELAY);
    }
    ...
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER_HOST),
                              (void **)&host);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER_PASSWORD),
                              (void **)&password);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER_MOUNTPOINT),
                              (void **)&mountpoint);
    ...
    ERROR_ACTION(TAG, sock == CONNECT_SOCKET_ERROR_RESOLVE, goto _error,
                 "Could not resolve host");
    ERROR_ACTION(TAG, sock == CONNECT_SOCKET_ERROR_CONNECT, goto _error,
    ...
    snprintf(buffer, BUFFER_SIZE,
             "SOURCE %s /%s" NEWLINE
             "Source-Agent: NTRIP %s/%s" NEWLINE NEWLINE,
             password, mountpoint, NTRIP_SERVER_NAME,
             &esp_app_get_description()->version[1]);

    int err = write(sock, buffer, strlen(buffer));
    ERROR_ACTION(TAG, err < 0, goto _error,
                 "Could not send request to caster: %d %s", errno,
                 strerror(errno));

    int len = read(sock, buffer, BUFFER_SIZE - 1);
    ERROR_ACTION(TAG, len <= 0, goto _error,
                 "Could not receive response from caster: %d %s", errno,
                 strerror(errno));
    buffer[len] = '\0';

    char *status = extract_http_header(buffer, "");
    ERROR_ACTION(TAG, status == NULL || !ntrip_response_ok(status),
                 free(status);
                 goto _error, "Could not connect to mountpoint: %s",
                 status == NULL ? "HTTP response malformed" : status);
    free(status);
...
void ntrip_server_init() {
  if (!config_get_bool1(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER_ACTIVE)))
    return;

  xTaskCreate(ntrip_server_task, "ntrip_server_task", 4096, NULL,
              TASK_PRIORITY_INTERFACE, &server_task);
}
