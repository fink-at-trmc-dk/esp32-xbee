...
static TaskHandle_t server1_task          = NULL;
static TaskHandle_t server2_task          = NULL;
static TaskHandle_t server3_task          = NULL;
...
static void ntrip_server_uart_handler(void *handler_args, esp_event_base_t base, int32_t length, void *buffer)
{
  ...
  if ( (event_bits & DATA_READY_BIT) == 0 )
  {
    ...
    if ( event_bits & DATA_SENT_BIT )
      ESP_LOGI(
          TAG, "Data received by UART, will now reconnect to caster if "
               "disconnected");
  }
  data_keep_alive = 0;

  // Ignore if caster is not connected and ready for data
  if ( (event_bits & CASTER_READY_BIT) == 0 )
    return;

  // Caster is connected and some data will be sent
  if ( (event_bits & DATA_SENT_BIT) == 0 )
    xEventGroupSetBits(server_event_group, DATA_SENT_BIT);

  int sent = write(sock, buffer, length);
  if ( sent < 0 )
  {
    destroy_socket(&sock);
    vTaskResume(server1_task);
    vTaskResume(server2_task);
    vTaskResume(server3_task);
  }
  else
  {
...
static void ntrip_server_sleep_task(void *ctx)
{
  ...
  while ( true )
  {
    ...
    if ( data_keep_alive == NTRIP_KEEP_ALIVE_THRESHOLD )
    {
      ...
      ESP_LOGW(
          TAG,
          "No data received by UART in %d seconds, will not reconnect to "
          "caster if disconnected",
          NTRIP_KEEP_ALIVE_THRESHOLD / 1000);
    }
...
static void ntrip_server_task(void *ctx)
{
  ...
  config_color_t status_led_color = config_get_color(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER1_COLOR));
  if ( status_led_color.rgba != 0 )
    status_led = status_led_add(status_led_color.rgba, STATUS_LED_FADE, 500, 2000, 0);
  if ( status_led != NULL )
  ...
  while ( true )
  {
    ...
    uint16_t port = config_get_u16(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER1_PORT));
    config_get_primitive(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER1_PORT), &port);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER1_HOST), (void **) &host);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER1_PASSWORD), (void **) &password);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER1_MOUNTPOINT), (void **) &mountpoint);
    ...
    snprintf(
        buffer, BUFFER_SIZE, "SOURCE %s /%s" NEWLINE "Source-Agent: NTRIP %s/%s" NEWLINE NEWLINE, password, mountpoint, NTRIP_SERVER_NAME,
    ...
    ERROR_ACTION(TAG, status == NULL || !ntrip_response_ok(status), free(status);
                 goto _error, "Could not connect to mountpoint: %s", status == NULL ? "HTTP response malformed" : status);
    free(status);
...
{
  if ( !config_get_bool1(CONF_ITEM(KEY_CONFIG_NTRIP_SERVER1_ACTIVE)) )
    return;

  xTaskCreate(ntrip_server_task, "ntrip_server1_task", 4096, NULL, TASK_PRIORITY_INTERFACE, &server1_task);

  // xTaskCreate(ntrip_server_task, "ntrip_server2_task", 4096, NULL, TASK_PRIORITY_INTERFACE, &server2_task);

  // xTaskCreate(ntrip_server_task, "ntrip_server3_task", 4096, NULL, TASK_PRIORITY_INTERFACE, &server3_task);
}
