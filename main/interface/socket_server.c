...
static bool socket_address_equal(struct sockaddr_in6 *a,
...
static socket_client_t *socket_client_add(int sock, struct sockaddr_in6 addr,
                                          int socktype) {
  socket_client_t *client = malloc(sizeof(socket_client_t));
  *client = (socket_client_t){.socket = sock, .addr = addr, .type = socktype};
  ...
  uart_nmea("$PESP,SOCK,SRV,%s,CONNECTED,%s", SOCKTYPE_NAME(socktype),
            addr_str);
...
static void socket_client_remove(socket_client_t *socket_client) {
  ...
  ESP_LOGI(TAG, "Disconnected %s client %s", SOCKTYPE_NAME(socket_client->type),
           addr_str);
  uart_nmea("$PESP,SOCK,SRV,%s,DISCONNECTED,%s",
            SOCKTYPE_NAME(socket_client->type), addr_str);
  ...
  if (status_led != NULL && SLIST_EMPTY(&socket_client_list))
    status_led->flashing_mode = STATUS_LED_STATIC;
}

static void socket_server_uart_handler(void *handler_args,
                                       esp_event_base_t base, int32_t length,
                                       void *buf) {
  ...
  SLIST_FOREACH_SAFE(client, &socket_client_list, next, client_tmp) {
    ...
    if (sent < 0) {
      ESP_LOGE(TAG, "Could not write to %s socket: %d %s",
               SOCKTYPE_NAME(client->type), errno, strerror(errno));
...
static int socket_init(int socktype, int port) {
  ...
  ERROR_ACTION(TAG, sock < 0, return -1, "Could not create %s socket: %d %s",
               SOCKTYPE_NAME(socktype), errno, strerror(errno))

  int reuse = 1;
  int err = setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse));
  ERROR_ACTION(TAG, err != 0, close(sock);
               return -1, "Could not set %s socket options: %d %s",
                      SOCKTYPE_NAME(socktype), errno, strerror(errno))
  ...
  ERROR_ACTION(TAG, err != 0, close(sock);
               return -1, "Could not bind %s socket: %d %s",
                      SOCKTYPE_NAME(socktype), errno, strerror(errno))

  ESP_LOGI(TAG, "%s socket listening on port %d", SOCKTYPE_NAME(socktype),
           port);
...
static esp_err_t socket_tcp_init() {
  ...
  ERROR_ACTION(TAG, err != 0, destroy_socket(&sock_tcp);
               return ESP_FAIL, "Could not listen on TCP socket: %d %s", errno,
                      strerror(errno))
...
static esp_err_t socket_tcp_accept() {
  ...
  int sock =
      accept(sock_tcp, (struct sockaddr *)&source_addr, (socklen_t *)&addr_len);
  ERROR_ACTION(TAG, sock < 0, return ESP_FAIL,
               "Could not accept new TCP connection: %d %s", errno,
               strerror(errno))
...
static bool socket_udp_has_client(struct sockaddr_in6 *source_addr) {
  ...
  SLIST_FOREACH(client, &socket_client_list, next) {
    ...
    if (socket_address_equal(source_addr, client_addr))
      return true;
  }
...
static esp_err_t socket_udp_client_accept(struct sockaddr_in6 source_addr) {
  if (socket_udp_has_client(&source_addr))
    return ESP_OK;

  int sock = socket(PF_INET6, SOCK_DGRAM, 0);
  ERROR_ACTION(TAG, sock < 0, return sock,
               "Could not create client UDP socket: %d %s", errno,
               strerror(errno))

  int reuse = 1;
  int err = setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse));
  ERROR_ACTION(TAG, err != 0, destroy_socket(&sock);
               return err, "Could not set client UDP socket options: %d %s",
  ...
  ERROR_ACTION(TAG, err != 0, destroy_socket(&sock);
               return err, "Could not bind client UDP socket: %d %s", errno,
                      strerror(errno))

  err = connect(sock, (struct sockaddr *)&source_addr, sizeof(source_addr));
  ERROR_ACTION(TAG, err != 0, destroy_socket(&sock);
               return err, "Could not connect client UDP socket: %d %s", errno,
                      strerror(errno))
...
static esp_err_t socket_udp_accept() {
  ...
  int len;
  while ((len = recvfrom(sock_udp, buffer, BUFFER_SIZE, MSG_DONTWAIT,
                         (struct sockaddr *)&source_addr, &socklen)) > 0) {
    // Multiple connections could have been made at once, so accept for every
  ...
  if (len < 0 && errno != EWOULDBLOCK) {
    ESP_LOGE(TAG, "Unable to receive UDP connection: %d %s", errno,
             strerror(errno));
    return ESP_FAIL;
  }
...
static void socket_clients_receive(fd_set *socket_set) {
  ...
  SLIST_FOREACH_SAFE(client, &socket_client_list, next, client_tmp) {
    ...
    // Receive until nothing left to receive
    int len;
    while ((len = recv(client->socket, buffer, BUFFER_SIZE, MSG_DONTWAIT)) >
           0) {
      stream_stats_increment(stream_stats, len, 0);
...
static void socket_server_task(void *ctx) {
  ...
  config_color_t status_led_color =
      config_get_color(CONF_ITEM(KEY_CONFIG_SOCKET_SERVER_COLOR));
  ...
  while (true) {
    ...
    while (true) {
      ...
      ERROR_ACTION(TAG, err < 0, goto _error,
                   "Could not select socket to receive from: %d %s", errno,
                   strerror(errno))

      // Accept new connections
      if (FD_ISSET(sock_tcp, &socket_set))
        socket_tcp_accept();
      if (FD_ISSET(sock_udp, &socket_set))
        socket_udp_accept();
...
void socket_server_init() {
  if (!config_get_bool1(CONF_ITEM(KEY_CONFIG_SOCKET_SERVER_ACTIVE)))
    return;

  xTaskCreate(socket_server_task, "socket_server_task", 4096, NULL,
