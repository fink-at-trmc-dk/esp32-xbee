...
static SLIST_HEAD(caster_clients_list_t,
                  ntrip_caster_client_t) caster_clients_list;

static void ntrip_caster_client_remove(ntrip_caster_client_t *caster_client) {
  struct sockaddr_in6 client_addr;
  socklen_t socklen = sizeof(client_addr);
  int err = getpeername(caster_client->socket, (struct sockaddr *)&client_addr,
                        &socklen);
  char *addr_str =
      err != 0 ? "UNKNOWN" : sockaddrtostr((struct sockaddr *)&client_addr);

  uart_nmea("$PESP,NTRIP,CST,CLIENT,DISCONNECTED,%s", addr_str);

  destroy_socket(&caster_client->socket);

  SLIST_REMOVE(&caster_clients_list, caster_client, ntrip_caster_client_t,
               next);
  free(caster_client);

  if (status_led != NULL && SLIST_EMPTY(&caster_clients_list))
    status_led->flashing_mode = STATUS_LED_STATIC;
}

static void ntrip_caster_uart_handler(void *handler_args, esp_event_base_t base,
...
static int ntrip_caster_socket_init() {
  ...
  ERROR_ACTION(TAG, sock < 0, return sock, "Could not create TCP socket: %d %s",
               errno, strerror(errno))

  int reuse = 1;
  int err = setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse));
  ERROR_ACTION(TAG, err != 0, destroy_socket(&sock);
               return err, "Could not set TCP socket options: %d %s", errno,
                      strerror(errno))

  struct sockaddr_in6 srv_addr = {.sin6_family = PF_INET6,
                                  .sin6_addr = IN6ADDR_ANY_INIT,
                                  .sin6_port = htons(port)};

  err = bind(sock, (struct sockaddr *)&srv_addr, sizeof(srv_addr));
  ERROR_ACTION(TAG, err != 0, destroy_socket(&sock);
               return err, "Could not bind TCP socket: %d %s", errno,
                      strerror(errno))

  err = listen(sock, 1);
  ERROR_ACTION(TAG, err != 0, destroy_socket(&sock);
               return err, "Could not listen on TCP socket: %d %s", errno,
                      strerror(errno))
...
static void ntrip_caster_task(void *ctx) {
  ...
  config_color_t status_led_color =
      config_get_color(CONF_ITEM(KEY_CONFIG_NTRIP_CASTER_COLOR));
  if (status_led_color.rgba != 0)
    status_led =
        status_led_add(status_led_color.rgba, STATUS_LED_STATIC, 500, 2000, 0);
  ...
  while (true) {
    ...
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_CASTER_USERNAME),
                              (void **)&username);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_CASTER_PASSWORD),
                              (void **)&password);
    config_get_str_blob_alloc(CONF_ITEM(KEY_CONFIG_NTRIP_CASTER_MOUNTPOINT),
                              (void **)&mountpoint);
    ...
    while (true) {
      ...
      sock_client =
          accept(sock, (struct sockaddr *)&source_addr, (socklen_t *)&addr_len);
      ERROR_ACTION(TAG, sock_client < 0, goto _error,
                   "Could not accept connection: %d %s", errno, strerror(errno))

      int len = read(sock_client, buffer, BUFFER_SIZE - 1);
      ERROR_ACTION(TAG, len <= 0, continue,
                   "Could not receive from client: %d %s", errno,
                   strerror(errno))
      buffer[len] = '\0';
      ...
      ERROR_ACTION(
          TAG, mountpoint_path == NULL,
          {
            char *response = "HTTP/1.1 405 Method Not Allowed" NEWLINE
                             "Allow: GET" NEWLINE NEWLINE;

            int err = write(sock_client, response, strlen(response));
            if (err < 0)
              ESP_LOGE(TAG, "Could not send response to client: %d %s", errno,
                       strerror(errno));
      ...
      // Move pointer to name of mountpoint, or empty string if sourcetable
      // request
      ...
      char *basic_authentication =
          strlen(username) == 0 ? NULL
                                : http_auth_basic_header(username, password);
      char *authorization_header =
          extract_http_header(buffer, "Authorization:");
      bool authenticated =
      ...
      bool ntrip_agent = user_agent_header == NULL ||
                         strcasestr(user_agent_header, "NTRIP") != NULL;
      ...
      if (print_sourcetable) {
        ...
        snprintf(stream, sizeof(stream),
                 "STR;%s;;;;;;;;0.00;0.00;0;0;;none;%c;N;0;" NEWLINE
                 "ENDSOURCETABLE",
                 mountpoint, strlen(username) == 0 ? 'N' : 'B');

        snprintf(buffer, BUFFER_SIZE,
                 "%s 200 OK" NEWLINE "Server: NTRIP %s/%s" NEWLINE
                 "Content-Type: text/plain" NEWLINE "Content-Length: %d" NEWLINE
                 "Connection: close" NEWLINE NEWLINE "%s",
                 ntrip_agent ? "SOURCETABLE" : "HTTP/1.0", NTRIP_CASTER_NAME,
                 &esp_app_get_description()->version[1], strlen(stream),
                 stream);
      ...
      if (!authenticated) {
        ...
        snprintf(buffer, BUFFER_SIZE,
                 "HTTP/1.0 401 Unauthorized" NEWLINE "Server: %s/1.0" NEWLINE
      ...
      ERROR_ACTION(TAG, err < 0, continue,
                   "Could not send response to client: %d %s", errno,
                   strerror(errno))
      ...
      // Socket will now be dealt with by ntrip_caster_uart_handler, set to -1
      // so it doesn't get destroyed
      sock_client = -1;

      if (status_led != NULL)
        status_led->flashing_mode = STATUS_LED_FADE;
...
void ntrip_caster_init() {
  ...
  xTaskCreate(ntrip_caster_task, "ntrip_caster_task", 4096, NULL,
              TASK_PRIORITY_INTERFACE, NULL);
}